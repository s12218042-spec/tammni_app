...
class TemporaryParentChatPage extends StatefulWidget {
  ...
  @override
  State<TemporaryParentChatPage> createState() =>
      _TemporaryParentChatPageState();
}

class _TemporaryParentChatPageState extends State<TemporaryParentChatPage> {
  ...
  bool _isMessageForThisConversation(Map<String, dynamic> data) {
    ...
    final wantedRole = widget.targetRole.trim().toLowerCase();
    final wantedUid = widget.targetUid.trim();

    final fromTemporaryToTarget =
        fromRole == 'temporary_parent' && targetRole == wantedRole;

    final fromTargetToTemporary =
        fromRole == wantedRole && targetRole == 'temporary_parent';

    final targetUidMatches = wantedUid.isEmpty ||
        wantedUid == 'admin' ||
        messageTargetUid.isEmpty ||
        messageTargetUid == wantedUid;

    return (fromTemporaryToTarget || fromTargetToTemporary) &&
        targetUidMatches;
  }
    ...
    return Container(
      ...
      child: Row(
        children: [
          CircleAvatar(
            radius: 26,
            backgroundColor: _targetColor.withOpacity(0.14),
            child: Icon(_targetIcon, color: _targetColor),
          ),
  ...
  Widget _buildMessageBubble(Map<String, dynamic> data) {
    final fromRole = _cleanText(data['fromRole']).toLowerCase();
    final isMe = fromRole == 'temporary_parent';
