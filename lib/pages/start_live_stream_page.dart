...
class LiveStreamService {
  ...
  static const String nurseryStationRoomId = 'nursery_main_stream';
  static const int heartbeatIntervalSeconds = 20;
  static const int heartbeatMaxAgeSeconds = 90;
  ...
  Future<Map<String, dynamic>?> getActiveNurseryStationStream() async {
    final doc = await _firestore
        .collection('live_streams')
        .doc(nurseryStationRoomId)
        .get();
    ...
    if (!_hasFreshHeartbeat(data)) return null;
  ...
  bool _hasFreshHeartbeat(Map<String, dynamic> data) {
    ...
    final heartbeatDate = _dateFromDynamic(data['lastHeartbeatAt']);
    if (heartbeatDate == null) return false;

    final difference = DateTime.now().difference(heartbeatDate).inSeconds;
    return difference <= heartbeatMaxAgeSeconds;
  }
  ...
  void _startHeartbeat(String roomId) {
    ...
    Future<void> updateHeartbeat() async {
      try {
        await _firestore.collection('live_streams').doc(roomId).set({
          'status': 'active',
          'isActive': true,
        ...
        debugPrint('live stream heartbeat error: $e');
      }
    }
  ...
  Future<String> startNurseryStationStream({
    String title = 'بث مباشر من الحضانة',
    String startedByUid = '',
    String startedByName = 'محطة البث',
    String startedByRole = 'nursery_station',
    String section = 'Nursery',
    String group = '',
  }) async {
    try {
      ...
      final roomRef =
          _firestore.collection('live_streams').doc(nurseryStationRoomId);

      _currentRoomId = nurseryStationRoomId;
      ...
      await roomRef.set({
        ...
        'status': 'active',
        'isActive': true,
        'roomId': nurseryStationRoomId,
        'scope': 'nursery',
        'stationType': 'nursery_tablet',
        'requestId': '',
        'liveStreamRequestId': '',
        'childId': '',
        'childName': '',
        ...
        'startedByName': startedByName.trim().isEmpty
            ? 'محطة البث'
            : startedByName.trim(),
        ...
        'maxViewers': 3,
        'notifyParents': false,
        'isIndividualStream': false,
        'isChildSpecificStream': false,
      ...
      _startHeartbeat(nurseryStationRoomId);
      _listenForRoomEnded(roomRef);
      _listenForViewers(roomRef);

      return nurseryStationRoomId;
    } catch (e) {
      debugPrint('startNurseryStationStream error: $e');
      rethrow;
    }
  }

  Future<void> stopNurseryStationStream() async {
    await endLiveStream(roomId: nurseryStationRoomId);
  }
  ...
  Future<void> joinLiveStream({
    ...
    try {
      ...
      if (roomData['status'] != 'active' || !_hasFreshHeartbeat(roomData)) {
